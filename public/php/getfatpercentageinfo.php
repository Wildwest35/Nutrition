...
//Purpose: Collect all user's fat percentage info
    ...
    if(isApiToken()) {
        if(isset($_POST['idLang']) && isset($_POST['limit']) && isset($_POST['offset'])) {
            ...
            if(!preg_match(Numeric(), $id) || !preg_match(Numeric(), $idLang) || !preg_match(Numeric(), $limits) || !preg_match(Numeric(), $offset)) {
                ...
                $query = $con->prepare("SELECT  `fatpercentage`.`id` as `idRegister`, `fatpercentage`.`fatPercentage` as `number`, `fatpercentage`.`fatDate` as `date`
                                        FROM `fatpercentage`
                                        WHERE `fatpercentage`.`idUser` = :idUser
                                        ORDER BY `fatpercentage`.`fatDate` DESC
                                        LIMIT :offset, :limits");
