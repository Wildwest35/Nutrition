<?php
//Access: Everyone
//Purpose: Collect all weight bone info
    ...
    if(isApiToken()) {
        if(isset($_POST['idLang']) && isset($_POST['limit']) && isset($_POST['offset'])) {
            ...
            if(!preg_match(Numeric(), $id) || !preg_match(Numeric(), $idLang) || !preg_match(Numeric(), $limits) || !preg_match(Numeric(), $offset)) {
                ...
                $query = $con->prepare("SELECT `weightbone`.`id` as `idRegister`, `weightbone`.`weightBone` as `number`, `weightbone`.`weightBoneDate` as `date`
                                        FROM `weightbone`
                                        WHERE `weightbone`.`idUser` = :idUser
                                        ORDER BY `weightbone`.`weightBoneDate` DESC
                                        LIMIT :offset, :limits");
