        ...
        if(isset($_POST['username']) && isset($_POST['newVal']) && isset($_POST['date'])) {
            ...
            if(!preg_match(AlphaNumeric(), $username) || !preg_match(Numeric(), $id)) {
                ...
                if($query->execute()) {
                    ...
                    if(!empty($row)) {
                        $query1 = $con->prepare("UPDATE `dailysummaries` SET `dailysummaries`.`note` = :note WHERE `dailysummaries`.`idDailyConsumptionGoals` = :idDailyConsumptionGoals && `dailysummaries`.`dailySummariesDate` = :dailySummariesDate");
                        ...
                        if($query1->execute()) {
                            ...
                            echo json_encode(['status' => 'error', 'data' => '* Κάτι πήγε λάθος! Η σημείωση δεν άλλαξε!']);                       
                        }
                        die();
                    } else {
