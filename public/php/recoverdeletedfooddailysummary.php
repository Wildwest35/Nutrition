...
//Purpose: Update daily summaries after a delete from daily eatings
            ...
            if(!preg_match(Numeric(), $id) || !preg_match(Numeric(), $usernameId)) {
                ...
                if($query->execute()) {                   
                    ...
                    while($row = $query->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        $query1 = $con->prepare("SELECT `dailysummaries`.`idDailyConsumptionGoals`, `dailysummaries`.`calories`, `dailysummaries`.`protein`, `dailysummaries`.`carb`, `dailysummaries`.`fat`, `dailysummaries`.`fiber`, `dailysummaries`.`saturated`, `dailysummaries`.`unsaturated`, `dailysummaries`.`sugar`, `dailysummaries`.`bitamins`, `dailysummaries`.`dailySummariesDate`
                                                 FROM `dailysummaries` 
                        ...
                        if($query1->execute()) {
                            ...
                            while($row1 = $query1->fetch(PDO::FETCH_ASSOC)) {
                                ...
                                $query2->bindValue(":idDailyConsumptionGoals", $row1['idDailyConsumptionGoals']);
                                $query2->bindValue(":dailySummariesDate", $row['dailyEatingsDate']);

    
                                if($query2->execute()) {
                                    if($counter == $row_count) {
                                        if($counter1 == $row1_count) {
