...
//Purpose: Update kilos
    ...
    if(isApiToken()) {
        if(isset($_POST['username']) && isset($_POST['newVal']) && isset($_POST['sendLang']) && isset($_POST['age']) && isset($_POST['height']) && isset($_POST['weight']) && isset($_POST['isIncreaseWeight']) && isset($_POST['exercise']) && isset($_POST['sex']) && isset($_POST['fd'])) {
            include 'connect.php';
            ...
            if(!preg_match(AlphaNumeric(), $username) || !preg_match(Numeric(), $kilos) || !preg_match(Numeric(), $id) || !preg_match(Numeric(), $age) || !preg_match(Numeric(), $height) || !preg_match(Numeric(), $weight) || !preg_match(Numeric(), $isIncreaseWeight) || !preg_match(Numeric(), $exercise) || !preg_match(Numeric(), $sex) || !preg_match(Numeric(), $fd)) {
                echo json_encode(['status' => 'error', 'data' => 'Error Code: #704']);
                die();
            } else {
                $query = $con->prepare("UPDATE `users` SET `users`.`kilos` = :kilos WHERE `users`.`username` = :newVal && `users`.`id` = :id");
            
                $query->bindValue(':kilos', $kilos);
                ...
                if($query->execute()) {
                    ...
                    if($query1->execute()) {
                        echo json_encode(['status' => 'ok', 'data' => $kilos]);
                    } else {
                        echo json_encode(['status' => 'error', 'data' => 'false2']);
                    }
                } else {
                    echo json_encode(['status' => 'error', 'data' => 'Κάτι πήγε λάθος! Το Φύλο δεν άλλαξε!']);                       
                }
                die();
            }
            die();     
