    ...
    if(isApiToken()) {
        if(isset($_POST['username']) && isset($_POST['numLang']) && isset($_POST['search']) && isset($_POST['limit']) && isset($_POST['offset'])) {
            ...
            if(!preg_match(AlphaNumeric(), $usernames) || !preg_match(Numeric(), $id) || !preg_match(Numeric(), $numLang) || !preg_match(Numeric(), $limits) || !preg_match(Numeric(), $offset)) {
                ...
                $query = $con->prepare("SELECT `createdfood`.`idFood` as `idFood`, `users`.`username`, `translationfoodname`.`translationFoodName` as `foodName`, `foods`.`imgPath` as `imgPath`, `foods`.`imgName` as `imgName`, `foods`.`imgHash` as `imgHash`, `translationfoodcategory`.`translationFoodCategory` as `category`, `translationunitname`.`translationUnitName` as `unitName`, `createdfood`.`isFavourite` as `isFavourite`, `foods`.`portion`, `foods`.`calories` as `calories`, `foods`.`protein` as `protein`, `foods`.`carb` as `carb`, `foods`.`fat` as `fat`, `foods`.`fiber` as `fiber`, `foods`.`saturated` as `saturated`, `foods`.`unsaturated` as `unsaturated`, `foods`.`sugar` as `sugar`, `foods`.`bitamins` as `bitamins`, `unitname`.`id` as `idUnit`, `foodcategory`.`id` as `idCategory`, `createdfood`.`id` as `idCreatedFood`, `createdfood`.`idUser` as `idUser`
                                        FROM `createdfood`
