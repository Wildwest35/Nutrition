    ...
    if(isApiToken()) {
        if(isset($_POST['id'])) {
            ...
            if(!preg_match(Numeric(), $id) || !preg_match(Numeric(), $usernameId)) {
                ...
                $query = $con->prepare("SELECT `unitname`.`id` FROM `unitname` WHERE `unitname`.`id` = :id");
                ...
                if($query->execute()) {
                    ...
                    if(!empty($row)) {
                        $query1 = $con->prepare("DELETE FROM `unitname` WHERE `unitname`.`id` = :id");
