    ...
    if(isApiToken()) {
        if(isset($_POST['username']) && isset($_POST['numLang']) && isset($_POST['search']) && isset($_POST['limit']) && isset($_POST['offset'])) {
            ...
            if(!preg_match(AlphaNumeric(), $usernames) || !preg_match(Numeric(), $id) || !preg_match(Numeric(), $numLang) || !preg_match(Numeric(), $limits) || !preg_match(Numeric(), $offset)) {
                ...
                $query = $con->prepare("SELECT `foodcategory`.`id` as `idCategory`, `unitname`.`id` as `idUnit`, `createdfood`.`idFood` as `idFood`, `translationunitname`.`translationUnitName` as `unitName`, `createdfood`.`isFavourite` as `isFavourite`, `translationfoodname`.`translationFoodName` as `foodName`, `translationfoodcategory`.`translationFoodCategory` as `category`, `foods`.`imgPath` as `imgPath`, `foods`.`imgName` as `imgName`, `foods`.`imgHash` as `imgHash`, `foods`.`portion`, `foods`.`calories` as `calories`, `foods`.`protein` as `protein`, `foods`.`carb` as `carb`, `foods`.`fat` as `fat`, `foods`.`fiber` as `fiber`, `foods`.`saturated` as `saturated`, `foods`.`unsaturated` as `unsaturated`, `foods`.`sugar` as `sugar`, `foods`.`bitamins` as `bitamins`
                                        FROM `createdfood`
                                        ...
                                        WHERE `createdFood`.`idUser` = :id && `translationfoodname`.`idLang` = :idLang && `translationfoodcategory`.`idLang` = :idLang1 && `translationunitname`.`idLang` = :idLang2 && (`foods`.`belongCategory` = 2 || `foods`.`belongCategory` = 3)
                                        && `translationfoodname`.`translationFoodName` LIKE :search
