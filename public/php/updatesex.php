...
//Purpose: Update sex
    ...
    if(isApiToken()) {
        if(isset($_POST['username']) && isset($_POST['newVal']) && isset($_POST['sendLang']) && isset($_POST['age']) && isset($_POST['height']) && isset($_POST['weight']) && isset($_POST['isIncreaseWeight']) && isset($_POST['exercise']) && isset($_POST['kilos']) && isset($_POST['fd'])) {
            include 'connect.php';
            ...
            if(!preg_match(AlphaNumeric(), $username) || !preg_match(Numeric(), $sex) || !preg_match(Numeric(), $id) || !preg_match(Numeric(), $age) || !preg_match(Numeric(), $height) || !preg_match(Numeric(), $weight) || !preg_match(Numeric(), $isIncreaseWeight) || !preg_match(Numeric(), $exercise) || !preg_match(Numeric(), $kilos) || !preg_match(Numeric(), $fd)) {
                ...
                $query = $con->prepare("UPDATE `users` SET `users`.`sex` = :sex WHERE `users`.`username` = :newVal && `users`.`id` = :id");
                ...
                if($query->execute()) {
                    ...
                    if($query1->execute()) {
                        echo json_encode(['status' => 'ok', 'data' => $sex]);
                    } else {
